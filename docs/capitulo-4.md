...
**Pantalla de Login ("Bienvenido de vuelta"):** Don Lucho llega a la pantalla de acceso con los campos "Correo electrónico" y "Contraseña", la casilla "Mantener sesión iniciada", el enlace "Olvidé mi contraseña" y el botón "Continuar con Google". Presiona el enlace "¿No tienes una cuenta todavía? Regístrate aquí". → El formulario cambia a la tarjeta "Crear cuenta" (transición sin cambio de URL, solo alternancia de componentes).
 
**Formulario de Registro ("Activa tu acceso"):** Don Lucho completa "Nombre del negocio o responsable", "Correo electrónico", "Contraseña" y "Confirmar contraseña", y acepta la casilla "Acepto los términos y el uso de datos del servicio". Presiona "Crear cuenta". Si los datos son válidos y el email no está registrado (US-56, Scenario 1): el sistema crea la cuenta, asigna el Plan Free automáticamente, envía un código de verificación al correo y despliega el modal "Verificar correo". Si el email ya existe (US-56, Scenario 2): aparece un mensaje de error inline debajo del campo de correo. Si la contraseña no cumple los requisitos (US-56, Scenario 3): se resaltan los campos inválidos.
 
**Verificación de correo (US-57):** En el modal "Activa tu cuenta", Don Lucho ingresa el código recibido en el campo "Código de verificación" y presiona "Verificar y activar cuenta"; si se equivocó de correo, presiona "Corregir correo" para editarlo antes de reenviar el código. Si el código es válido: la cuenta queda activa y la pantalla muestra la confirmación "Cuenta activada correctamente", con los botones "Ir al dashboard" y "Volver al inicio". Si el código expiró o es inválido: el modal permanece abierto mostrando el mensaje de error y permite solicitar un nuevo código.
 
**Pantalla de Login:** En visitas posteriores, Don Lucho ingresa email y contraseña. Si las credenciales son correctas (US-58, Scenario 1): el sistema genera un JWT y redirige al Dashboard. Si las credenciales son incorrectas (US-58, Scenario 2): aparece un mensaje de error genérico sin indicar qué campo falló. Si falló 5 veces (US-58, Scenario 3): la cuenta se bloquea y se envía notificación al email. Como alternativa, si Don Lucho prefiere usar Google (US-59): presiona "Continuar con Google" y completa el flujo OAuth. Si olvida su contraseña (US-60): presiona "Olvidé mi contraseña" para iniciar la recuperación mediante un enlace enviado a su correo.
...
**Desde el Sidebar → Módulo "Lotes":** El sistema carga el dashboard de lotes (US-13) con tres contadores ("Lotes totales", "Lotes vencidos", "Sin stock"), el buscador "Buscar Lotes..." (US-95), el botón "Crear lote" y una campanita de notificaciones. Si existen condiciones críticas (US-14): la campanita muestra un contador de alertas y, al presionarla, despliega el panel "Alerta de lotes" con una fila por cada caso —próximo a vencer, a punto de agotarse o sin stock— indicando el producto, el número de lote y el detalle (por ejemplo "vence en 5 días (15/4/2026)"). Debajo, cada producto se presenta en una tarjeta con su tipo, stock total, cantidad de lotes y el botón "Ver Detalles".

**Crear lote (US-09):** Don Lucho presiona "Crear lote". El modal "Crear Nuevo Lote" se despliega con el selector "Producto" (nombre · marca · tipo), "Cantidad (unidades)", "Fecha de Ingreso", "Fecha de Caducidad" y "Código QR" (se autocompleta con el código del producto si se deja en blanco). Presiona "Guardar". Si todos los datos son válidos (Scenario 1): el lote queda registrado y el contador "Lotes totales" y la tarjeta del producto se actualizan. Si deja campos obligatorios vacíos (Scenario 2): los campos quedan marcados con una "X" y el mensaje "Campo obligatorio incompleto", y el botón "Guardar" permanece deshabilitado.

**Ver detalles de lote (US-06):** Don Lucho presiona "Ver Detalles" en la tarjeta de un producto. Si la carga es exitosa (Scenario 1): el sistema muestra el banner de alerta correspondiente (cuando aplica) y la tabla de lotes del producto con #ID, Cantidad, Fecha de ingreso, Fecha de caducidad y Código QR, cada fila con su badge de estado (por ejemplo "Vencido") y los íconos de editar y eliminar, además del botón "Agregar Lote" para registrar otro lote del mismo producto. Si ocurre un error de comunicación con el servidor (Scenario 2): el sistema muestra el mensaje "No se pudo cargar los detalles en este momento. Inténtelo más tarde." con la opción de reintentar.

**Editar lote (US-02):** Don Lucho presiona el ícono de editar en la fila del lote. El modal "Edición de Lote" se abre con el "ID" bloqueado y los campos Cantidad, Fecha de Ingreso, Fecha de Caducidad y Código QR pre-cargados. Si ingresa un valor inválido (Scenario 2): el campo muestra el ícono "X" y el mensaje "Debe poner campo válido", y el botón "Guardar" permanece deshabilitado. Si los datos son válidos (Scenario 1): al presionar "Guardar" el lote se actualiza en la tabla.

**Eliminar lote (US-04):** Don Lucho presiona el ícono de eliminar (papelera) en la fila del lote. El sistema elimina el lote, muestra el mensaje de confirmación "Lote se ha eliminado correctamente" y actualiza el stock y el contador de lotes del producto.
...
**Desde el Home → Sidebar "Chatbot":** La tarjeta "Estado del Chatbot" del Home (US-71) ya adelanta el número vinculado y las conversaciones activas del día. Si no hay cuenta vinculada (US-37, Scenario 1): la pantalla "Chatbot" muestra el panel "Vincular WhatsApp Business" con el texto "Escanea el código QR desde tu app", el código QR y el aviso "El código expira en 60 segundos". Don Lucho abre WhatsApp Business en su teléfono y escanea el código. Si la vinculación es exitosa (US-37, Scenario 2): el sistema registra la sesión y la pantalla cambia al panel "Conversaciones", inicialmente vacío con el mensaje "Aún no hay conversaciones. Los chats de tus clientes aparecerán aquí cuando el chatbot reciba mensajes.". Si el código QR expira antes de ser escaneado (US-37, Scenario 3): el sistema descarta el código y genera uno nuevo automáticamente.

**Consultar estado de conexión (US-38):** En visitas posteriores al módulo, si la sesión sigue activa (Scenario 1): se muestra el listado de conversaciones con el número vinculado. Si la sesión expiró o fue cerrada desde el teléfono (Scenario 2): el sistema vuelve a mostrar el panel de vinculación por QR.

**Gestionar conversaciones (US-39, US-40):** La lista "Conversaciones" del panel izquierdo muestra cada chat con el nombre del cliente, la hora y el último mensaje, ordenados por el más reciente. Don Lucho hace clic en una conversación para ver el historial completo en el panel derecho, donde se distinguen los mensajes del cliente, las respuestas automáticas del bot (incluyendo el registro de pedidos y la recepción de capturas de pago) y los avisos de sistema (por ejemplo "Pedido #0008 registrado · instrucciones de pago enviadas"). Si desea responder manualmente: escribe en el campo de texto y presiona "Enviar". Si el campo está vacío al presionar "Enviar": el sistema no procesa el envío (US-40, Scenario 2).
...
**Desde el Home → Sidebar "Suscripción":** La pantalla "Plan actual y facturación" muestra el badge de estado ("PLAN FREE ACTIVO"), la tarjeta del "Plan Actual" y la tarjeta "Recomendado" del Plan Control (S/89, con toggle Mensual/Anual y el botón "Actualizar a Plan Control"), además de los paneles "Límites disponibles" (Productos, Lotes activos, Usuarios), "Método de pago y datos fiscales" y "Actividad de la suscripción" con su botón "Descargar historial" (US-20, US-21, US-27).

**Proceso de suscripción (US-16, US-17, US-18):** Don Lucho presiona "Actualizar a Plan Control". Se despliega el modal "Actualizar a Plan Control" con cuatro pasos: "PLAN" (confirma el Plan Control y su ciclo de cobro), "FACTURACIÓN" (RUC, Razón social, Correo de facturación y Distrito), "PAGO" (muestra la tarjeta registrada o el mensaje "Listo para activar" junto al total a cobrar) y "ACTIVACIÓN". Don Lucho presiona "Pagar y activar suscripción". Si el cobro es aprobado (US-19, Scenario 1): el paso "ACTIVACIÓN" muestra "Suscripción activada — Plan Control activado" con el botón "Entendido", y el panel de Suscripción pasa a mostrar "Plan Control" como plan actual. Si el cobro falla (US-18, Scenario 2): aparece el mensaje de error dentro de la misma vista sin perder los datos ingresados.

**Gestión de suscripción activa (US-20 al US-24):** Con el Plan Control activo, Don Lucho puede presionar "Renovar suscripción" (US-22): el modal "Renovar suscripción" confirma "Renovación preparada" con la nueva fecha de vigencia. O puede presionar "Mantener plan" o solicitar la baja: el modal "Solicitar cancelación" muestra "Confirma la cancelación" con la fecha hasta la que se mantiene el acceso y el botón "Confirmar cancelación". Al confirmar: el modal "Cancelación programada" indica que la solicitud fue registrada y que el acceso premium continúa hasta el fin del periodo vigente, y la etiqueta de estado del panel cambia a "CANCELACIÓN PROGRAMADA". Al llegar la fecha de vencimiento: el sistema cancela automáticamente el plan premium y devuelve la cuenta al Plan Free (US-24).
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Generación y Autenticación de Cuenta.jpg" width="500"/> <img src="images/capitulo4/Canvas BC 6.jpg" width="500"/>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Perfil y Configuración.jpg" width="500"/> <img src="images/capitulo4/Canvas BC 2.jpg" width="500"/> </p>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Gestión y Proceso de suscripción.jpg" width="500"/> <img src="images/capitulo4/Canvas BC 7.jpg" width="500"/> </p>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Gestión de inventario.jpg" width="500"/> <img src="images/capitulo4/Canvas BC 1.jpg" width="500"/> </p>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Chatbot de WhatsApp.jpg" width="500"/> <img src="images/capitulo4/Canvas BC 5.jpg" width="500"/> </p>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Ventas.jpg" width="500"/> <img src="images/capitulo4/Canvas BC 3.jpg" width="500"/> </p>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - BC union.jpg" width="500"/> </p>
...
<p align="center"> <img src="images/capitulo4/Entreprenly - Flujo ChatbotBC.jpg" width="500"/></p>

<p align="center"><img src="images/capitulo4/Entreprenly - Flujo Ventas.jpg" width="500"/></p>

<p align="center"><img src="images/capitulo4/Entreprenly - Flujo Completo.jpg" width="500"/></p>

<p align="center"><img src="images/capitulo4/Entreprenly - Flujo Gestion de inventario.jpg" width="500"/></p>

<p align="center"><img src="images/capitulo4/Entreprenly - Flujo Gestion y Proceso de suscripcion.jpg" width="500"/></p>

<p align="center"><img src="images/capitulo4/Entreprenly - Flujo Perfil y Configuracion.jpg" width="500"/></p>

### 4.6.2. Software Architecture Context Diagram

A continuación, se presenta el System Context Diagram del sistema Entreprenly. En este diagrama se identifica al actor principal, denominado "Emprendedor", quien interactúa con la plataforma a través de la aplicación web (Vue 3). Asimismo, se muestra el único sistema externo integrado con la solución: el WhatsApp Bridge, encargado de enviar y recibir mensajes de WhatsApp para el canal conversacional de ventas.
